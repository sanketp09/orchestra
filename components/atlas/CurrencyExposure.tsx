...
function BentoCard({
  ...
  return (
    <motion.div
      ...
      className={cn("rounded-[12px] p-4 border bg-[var(--surface-card)]", className)}
      style={{
...
export default function CurrencyExposure() {
  ...
  const severityColor = needsHuman ? tokens.danger : tokens.success;
      ...
      <AnimatePresence mode="wait">
        {screen === "loading" && (
          <motion.div key="loading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center min-h-[440px] gap-6">
            <div className="relative w-14 h-14">
              <div className="absolute inset-0 rounded-full border-2 border-[var(--muted)]" style={{ borderColor: tokens.muted }} />
              <motion.div
                className="absolute inset-0 rounded-full border-2 border-transparent border-t-[var(--accent)]"
                style={{ borderTopColor: tokens.accent }}
                animate={{ rotate: 360 }}
        ...
        {screen === "error" && (
          <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center min-h-[440px] gap-4 text-center">
            <div className="w-12 h-12 rounded-full bg-[color-mix(in_srgb,var(--danger)_14%,transparent)] flex items-center justify-center" style={{ backgroundColor: `color-mix(in srgb, ${tokens.danger} 14%, transparent)` }}>
              <span className="text-[var(--danger)] text-[20px] leading-none" style={{ color: tokens.danger }}>!</span>
            </div>
            <div>
            ...
            <button className="text-[13px] text-black bg-[var(--accent)] hover:brightness-110 transition-all rounded-[8px] px-4 py-2 font-semibold" style={{ background: tokens.accent }} onClick={retry}>
              Retry
            </button>
          </motion.div>
        )}
          ...
          <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <BentoCard delay={0 * 0.1} className="sm:col-span-2">
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge className="text-[10px] px-2 py-0.5 rounded-[6px] font-bold border-0 bg-[color-mix(in_srgb,var(--accent)_16%,transparent)] text-[var(--accent)]" style={{ backgroundColor: `color-mix(in srgb, ${tokens.accent} 16%, transparent)`, color: tokens.accent }}>
              ...
              <div className="h-[150px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={RATE_TREND} margin={{ top: 8, right: 16, bottom: 0, left: 0 }}>
                    ...
                    <ReferenceLine y={bookingRate} stroke={tokens.mutedForeground} strokeDasharray="4 4" strokeWidth={1.5} />
                    <Line
                      type="monotone"
                      dataKey="rate"
            ...
            <BentoCard delay={1 * 0.1} className="flex flex-col justify-between">
              ...
              <div className="mt-3 pt-3 border-t" style={{ borderColor: tokens.muted }}>
            ...
            <BentoCard delay={2 * 0.1} className="sm:col-span-3 flex items-start gap-4">
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge
                    className="text-[9px] px-1.5 py-0.5 rounded-[6px] font-bold border-0"
