...
function EmbossCard({ className, children }: { className?: string; children: React.ReactNode }) {
  return (
    <div
      className={cn("h-full w-full p-4 flex flex-col justify-between border", className)}
      style={{
        borderRadius: 12,
...
function WageAvailabilityChartCard({ mounted }: { mounted: boolean }) {
  ...
  return (
    <EmbossCard>
      ...
      <div className="flex-1 relative min-h-0 h-[80px]">
  ...
  return (
    <EmbossCard>
      ...
      <div className="flex-1 flex flex-col justify-center gap-2 my-2">
        ...
        <p className="text-xs leading-relaxed" style={{ color: tokens.foreground }}>
...
function TradeRow({
  ...
  return (
    <motion.div
      ...
      className="grid items-center gap-3 rounded-lg px-2 py-1 transition-colors hover:bg-white/5"
