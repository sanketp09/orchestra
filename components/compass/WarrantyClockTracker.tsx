...
// Root screen
// ---------------------------------------------------------------------------
export default function WarrantyClockTracker({ data }: { data?: any }) {
  ...
  const warranties = useMemo(() => {
    if (!data?.warranties) return WARRANTIES;
    return data.warranties.map((w: any) => ({
      decisionId: w.decision_id || w.decisionId,
      equipmentName: w.equipment_name || w.equipmentName,
      entityId: w.entity_id || w.entityId,
      installDate: w.install_date || w.installDate,
      warrantyPeriodMonths: w.warranty_period_months || w.warrantyPeriodMonths,
      warrantyEndDate: w.warranty_end_date || w.warrantyEndDate,
      nextServiceDue: w.next_service_due || w.nextServiceDue,
      daysRemaining: w.days_remaining ?? w.daysRemaining,
      priorityLevel: w.priority_level || w.priorityLevel,
      serviceDueSoon: w.service_due_soon ?? w.serviceDueSoon,
      manufacturerRef: w.manufacturer_ref || w.manufacturerRef,
      decisionMade: w.decision_made || w.decisionMade,
      outcome: w.outcome,
      reasoning: w.reasoning,
      evidence: (w.evidence || []).map((e: any) => ({
        source: e.source,
  ...
  const criticalCount = useMemo(() => {
    return warranties.filter((w: any) => w.priorityLevel === "critical").length;
  }, [warranties]);
        ...
        >
          <div>
            ...
            <h1 className="text-2xl font-semibold mt-1" style={{ color: tokens.foreground }}>
              {data?.project_name ?? "Austin Semiconductor Fab"}
            </h1>
          </div>
