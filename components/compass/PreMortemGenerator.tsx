...
export default function PreMortemGenerator({ data: propData }: { data?: any }) {
  ...
  // If propData is passed, normalize it and set state
  const normalizedPropData = useMemo(() => {
    if (!propData) return null;
    return {
      decision: {
        id: propData.decision?.id || "decision",
        label: propData.decision?.label || "Decision",
        vendor_name: propData.decision?.vendor_name || propData.decision?.vendorName || "Vendor",
        package: propData.decision?.package || "Package",
        package_value_usd: propData.decision?.package_value_usd ?? propData.decision?.packageValueUsd ?? 0,
      },
      linked_dependencies: propData.linked_dependencies || propData.linkedDependencies || [],
      scenarios: (propData.scenarios || []).map((s: any) => ({
        id: s.id,
        category: s.category,
        title: s.title,
        probability_pct: s.probability_pct ?? s.probabilityPct ?? 0,
        severity: s.severity,
        risk_score: s.risk_score ?? s.riskScore ?? 0,
        is_high_priority: s.is_high_priority ?? s.isHighPriority ?? false,
        schedule_days_impact: s.schedule_days_impact ?? s.scheduleDaysImpact ?? null,
        cost_usd_impact: s.cost_usd_impact ?? s.costUsdImpact ?? null,
        trigger: s.trigger,
        failure: s.failure,
        impact: s.impact,
        early_warning: s.early_warning || s.earlyWarning || "",
        mitigation: s.mitigation,
        evidence: (s.evidence || []).map((ev: any) => ({
          type: ev.type,
          label: ev.label,
        })),
        narrative_source: s.narrative_source || s.narrativeSource || "llm",
      })),
      summary: {
        scenarios_generated: propData.summary?.scenarios_generated ?? propData.summary?.scenariosGenerated ?? 0,
        high_priority_count: propData.summary?.high_priority_count ?? propData.summary?.highPriorityCount ?? 0,
        highest_probability_category: propData.summary?.highest_probability_category ?? propData.summary?.highestProbabilityCategory ?? "schedule",
        total_schedule_exposure_days: propData.summary?.total_schedule_exposure_days ?? propData.summary?.totalScheduleExposureDays ?? 0,
        total_cost_exposure_usd: propData.summary?.total_cost_exposure_usd ?? propData.summary?.totalCostExposureUsd ?? 0,
      },
      confidence: propData.confidence ?? 1.0,
      reasoning: propData.reasoning || "",
    } as PreMortemResponse;
  }, [propData]);

  useEffect(() => {
    if (normalizedPropData) {
      setData(normalizedPropData);
      setRunState("done");
      setRevealedCount(normalizedPropData.scenarios.length);
    }
  }, [normalizedPropData]);
