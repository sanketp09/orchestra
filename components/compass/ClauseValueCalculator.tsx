...
function ceilingFor(id: ClauseId, clauses: Record<ClauseId, ClauseDef>): number {
  switch (id) {
...
function protectedAmountFor(id: ClauseId, v: number, clauses: Record<ClauseId, ClauseDef>): number {
  switch (id) {
    case "liquidated_damages_cap":
      return Math.min(Math.max(v, 0), ceilingFor(id, clauses));
    case "escalation_clause":
      return (ESC_MATERIAL_PORTION * Math.max(0, ESC_WORST_CASE_PCT - v)) / 100;
    case "termination_right":
      return ceilingFor(id, clauses) - v * TERM_DAILY_LOCK_IN_COST;
    case "payment_term": {
      const extra = Math.max(0, v - PAY_BASELINE_DAYS);
      return ceilingFor(id, clauses) - PAY_QUADRATIC_COEFFICIENT * extra ** 2;
    }
    case "warranty":
...
function recommendedValueFor(id: ClauseId, clauses: Record<ClauseId, ClauseDef>): number {
  switch (id) {
    case "liquidated_damages_cap":
      return Math.round((ceilingFor(id, clauses) * LD_RECOMMENDED_CEILING_CAPTURE) / 1000) * 1000;
    case "escalation_clause":
...
function computeClauseValue(id: ClauseId, value: number, clauses: Record<ClauseId, ClauseDef>): ComputedValue {
  const ceiling = ceilingFor(id, clauses);
  const currentProtected = protectedAmountFor(id, value, clauses);
  const gap = Math.max(0, ceiling - currentProtected);
  const recommendedValue = recommendedValueFor(id, clauses);
  const recommendedProtected = protectedAmountFor(id, recommendedValue, clauses);
...
function EmbossedCard({ className, children, style }: { className?: string; children: React.ReactNode; style?: React.CSSProperties }) {
  return (
    <Card
      className={cn("rounded-[12px] border-0 bg-[var(--surface-card)] p-5", className)}
      style={{ boxShadow: "-6px -6px 14px rgba(255,255,255,0.65), 8px 8px 18px rgba(12,9,4,0.10)", ...style }}
    >
      {children}
    </Card>
  );
...
function ClauseSelector({
  ...
  clauses: Record<ClauseId, ClauseDef>;
  selected: ClauseId;
...
export default function ClauseValueCalculator({ data }: { data?: any }) {
  const clauses = React.useMemo(() => {
    if (!data?.clauses) return CLAUSES;
    const result: Record<ClauseId, ClauseDef> = { ...CLAUSES };
    for (const key of Object.keys(data.clauses) as ClauseId[]) {
      if (data.clauses[key]) {
        result[key] = {
          ...result[key],
          ...data.clauses[key],
  ...
  // Sync value when selected clause changes or clauses config updates
  React.useEffect(() => {
    setValue(clauses[clauseId].current);
  }, [clauseId, clauses]);
  ...
  const computed = React.useMemo(() => computeClauseValue(clauseId, value, clauses), [clauseId, value, clauses]);
  ...
  return (
    <div className="min-h-screen w-full bg-[var(--background)] p-6 font-[Inter,sans-serif] md:p-10">
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
        ...
        <ClauseSelector clauses={clauses} selected={clauseId} onSelect={handleSelectClause} />
