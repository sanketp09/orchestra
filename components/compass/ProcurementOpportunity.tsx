  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full max-w-[460px] mx-auto h-auto">
      ...
      {visibleOpportunities.map((o, i) => {
        const angle = (-90 + (360 / Math.max(opportunities.length, 1)) * ranked.findIndex((r) => r.id === o.id)) * (Math.PI / 180);
      ...
      {visibleOpportunities.map((o) => {
        ...
        const angle = (-90 + (360 / Math.max(opportunities.length, 1)) * rankIndex) * (Math.PI / 180);
        const x = cx + orbitRadius * Math.cos(angle);
        const y = cy + orbitRadius * Math.sin(angle);
...
export default function ProcurementOpportunity({ data }: { data?: any }) {
  ...
  const opportunities = useMemo(() => {
    if (!data?.opportunities) return OPPORTUNITIES;
    return data.opportunities.map((o: any) => ({
      ...
      savings: o.savings,
      savingsPct: o.savings_pct ?? o.savingsPct ?? 0,
  ...
  const totalPotentialValue = useMemo(() => {
    return ranked.reduce((sum, o) => sum + o.savings, 0);
  }, [ranked]);

  const portfolioCurrentCost = data?.portfolio_current_cost ?? PORTFOLIO_CURRENT_COST;
  ...
  const timingOpportunities = opportunities.filter((o: any) => o.type === "buy_early" || o.type === "wait");
    ...
    >
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          ...
          <h1 className="text-[22px] font-bold text-[var(--foreground)] leading-tight">{companyName}</h1>
      ...
      <AnimatePresence mode="wait">
        {screen === "idle" && (
          <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center min-h-[560px] gap-4">
            <OpportunityRadar opportunities={opportunities} ranked={ranked} portfolioCurrentCost={portfolioCurrentCost} visibleOpportunities={[]} onSelect={() => {}} sweeping={false} />
        ...
        {screen === "scanning" && (
          <motion.div key="scanning" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center min-h-[560px] gap-6">
            <OpportunityRadar opportunities={opportunities} ranked={ranked} portfolioCurrentCost={portfolioCurrentCost} visibleOpportunities={[]} onSelect={() => {}} sweeping />
          ...
          <motion.div key="results" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col gap-8">
            ...
            <div className="flex flex-col items-center gap-3">
              <OpportunityRadar
                ...
                ranked={ranked}
                portfolioCurrentCost={portfolioCurrentCost}
                ...
                {visibleCount >= ranked.length && (
                  <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="text-center">
                    ...
                    <p className="text-[32px] font-bold tabular-nums" style={{ color: "var(--success)" }}>
                      +<AnimatedNumber value={totalPotentialValue} prefix="$" />
                    </p>
                  </motion.div>
                )}
              </AnimatePresence>
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <Badge
                          ...
                          style={{ backgroundColor: `color-mix(in srgb, ${TYPE_META[o.type as OppType].color} 16%, transparent)`, color: TYPE_META[o.type as OppType].color }}
                        >
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                <Card className="bg-[var(--surface-floating)] border-[var(--muted)] shadow-[0_2px_6px_rgba(12,9,4,.06)] rounded-[10px] p-5">
                  ...
                  <p className="text-[12px] text-[var(--muted-foreground)] leading-relaxed mb-4">
                    Scanned {companyName}'s current procurement plan — {ranked.length} opportunities found across{" "}
                    {new Set(opportunities.flatMap((o: any) => o.projects)).size} projects, combining only line items that
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div>
                      ...
                      <p className="text-[14px] font-bold text-[var(--foreground)] tabular-nums">{money(portfolioCurrentCost)}</p>
                    </div>
                    <div>
                      <p className="text-[10px] uppercase tracking-wide text-[var(--muted-foreground)]">Potential value</p>
                      <p className="text-[14px] font-bold text-[var(--success)] tabular-nums">{money(totalPotentialValue)}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-[13px] font-bold text-[var(--foreground)]">{ranked[0]?.title || "N/A"}</p>
                    </div>
                  </div>
                </Card>
              </motion.div>
