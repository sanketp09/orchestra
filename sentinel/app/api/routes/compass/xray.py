...
async def _extract_document(document_text: str) -> ExtractedDocument:
    ...
    if not os.environ.get("ANTHROPIC_API_KEY"):
        # Fallback to local deterministic mock extraction for demo robustness
        text_lower = document_text.lower()
        vendor_name = "Meridian Steel Fabrication"
        if "titan" in text_lower:
            vendor_name = "Titan Fabricators"
        elif "coastal" in text_lower:
            vendor_name = "Coastal Bolt & Fastener"
        ...
        pricing = {
            "structural steel columns": 3190.0,
            "structural steel beams": 2690.0,
            "base plates": 450.0,
            "shop primer coating": 180.0,
            "erection and installation": 620.0
        }
        clauses = ["payment terms: net 60"]
        dependencies = ["structural steel erection depends on concrete foundation pour"]
        document_dates = ["2026-08-25"]
        
        if "net 30" in text_lower:
            clauses = ["payment terms: net 30"]
        elif "net 90" in text_lower:
            clauses = ["payment terms: net 90"]
        
        if "uncapped" in text_lower or "liquidated" in text_lower:
            clauses.append("liquidated damages: uncapped")
