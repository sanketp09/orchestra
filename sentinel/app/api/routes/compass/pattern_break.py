...
async def _extract_decision(decision_text: str, project_hint: Optional[str]) -> ExtractedDecision:
    ...
    if not os.environ.get("ANTHROPIC_API_KEY"):
        ...
        weights = DecisionWeights(cost=0.15, schedule=0.55, quality=0.25, risk=0.05)
        if prioritized == "cost":
            weights = DecisionWeights(cost=0.60, schedule=0.15, quality=0.15, risk=0.10)
        elif prioritized == "quality":
            weights = DecisionWeights(cost=0.15, schedule=0.15, quality=0.60, risk=0.10)
        elif prioritized == "risk":
            weights = DecisionWeights(cost=0.15, schedule=0.15, quality=0.10, risk=0.60)

        return ExtractedDecision(
            project_name=project_hint or "Austin Semiconductor Fab",
            mission_critical=True,
            trade_off_type="cost_vs_quality",
            weights=weights,
            prioritized=prioritized,
            summary=f"Subcontractor selection prioritizing {prioritized}."
        )
