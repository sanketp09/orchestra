...
(test_rag.py, separately, is the live-infra integration check called for by
the build prompt.)

Run with: pytest common/rag/tests/
"""
...
def test_chunk_text_respects_overlap_and_order():
    ...
    assert len(chunks) > 1
    assert all(len(c) <= 100 + 20 for c in chunks)  # splitter can slightly exceed size
