...
// ---------------------------------------------------------------------------
// Mirrors route.py's daily_briefing() output — 9 condensed tiles, one per
// Atlas feature, already sorted severity-first (matches backend exactly).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Minimal inline icon set — no external icon library, matches repo's
// no-emoji, line-icon convention.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Neumorphic card — matches Trustline's soft-depth bento treatment
// ---------------------------------------------------------------------------

function NeumorphicCard({ className, children }: { className?: string; children: React.ReactNode }) {
  return (
    <div
      className={cn("rounded-[12px] p-5 relative overflow-hidden border border-[var(--muted)]/40", className)}
      style={{
        background: "var(--surface-card)",
        boxShadow: "-4px -4px 10px rgba(255,255,255,0.65), 6px 6px 14px rgba(12,9,4,0.08)",
      }}
    >
      {children}
    </div>
...
function BriefingTile({ tile, index }: { tile: Tile; index: number }) {
  ...
  return (
    ...
    >
      <NeumorphicCard
        ...
        // subtle severity-tinted left accent, consistent with Trustline's badge palette
      >
        <div
...
export default function AtlasDailyBriefing() {
    ...
    >
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {TILES.map((tile, i) => (
            <BriefingTile key={tile.slug} tile={tile} index={i} />
          ))}
        </div>
      </div>
    </div>
