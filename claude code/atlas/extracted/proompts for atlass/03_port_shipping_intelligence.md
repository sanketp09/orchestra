# 3. Port & Shipping Intelligence — 2 files
...
**Grid layout (3 cards), focused on Shipment #4471's actual live route:**
- **Hero card (col-span-2, row-span-2, glass treatment):** a route progress visualization — 
  a horizontal path with waypoints (factory → departure port → transit → Port of Houston → 
  site), the shipment's current position marked and animated with a subtle pulse, and a 
  congestion indicator specifically at the Port of Houston waypoint if relevant (a colored 
  ring around that waypoint, --warning if congestion detected).
- **ETA card:** original ETA vs. current adjusted ETA, with the delta highlighted if 
  they differ, animated count.
- **Recommended Action card:** a specific, concrete recommendation ("Reroute via Port of 
  Beaumont — saves 4 days") only shown if congestion is currently flagged, otherwise show 
  a calm "On track" state — build both states, not just the alert state.
...
`GET /atlas/shipment-status/{shipment_id}`. Seed a realistic multi-waypoint route for 
shipment_4471 with a current position and a seeded congestion flag at the Houston waypoint. 
Deterministic ETA recalculation: base ETA plus a delay estimate derived from the congestion 
severity (simple lookup, no LLM). If an alternate-port option is seeded as available, 
compute the reroute's time savings via straightforward arithmetic on seeded transit-time 
data. `needs_human=True` whenever a reroute is recommended, since redirecting an in-transit 
shipment is a real decision, not something to auto-execute.
