"""
Tests for Feature 2's duplicate checker. This is pure deterministic logic
(no LangGraph, no LLM call), so everything here runs for real — the only
thing supplied externally is the embedding vectors themselves, since a real
OPENAI_API_KEY isn't available in CI. We construct embeddings by hand so the
cosine-similarity math in duplicate_checker.py is exercised for real.
"""
...
REBAR_VECTOR = _unit_vector(1536, hot_index=10)
REBAR_NEAR_DUPLICATE_VECTOR = _unit_vector(1536, hot_index=10, near=0.05)  # cosine sim > 0.85
UNRELATED_VECTOR = _unit_vector(1536, hot_index=800)  # cosine sim ~0.0 vs rebar
