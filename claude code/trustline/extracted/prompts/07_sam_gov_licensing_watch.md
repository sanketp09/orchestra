# 7. SAM.gov Exclusion & Licensing Watch — 2 files
...
**Grid layout (3 cards):**
- **Hero status card (col-span-2, the glass card):** a large, unmistakable compliance 
  status — a big colored status indicator (a soft glowing dot/ring in --success or 
  --danger) with "SAM.gov: Active & Eligible" or equivalent, plus the license status beside 
  it, plus a "Last verified Xs ago" line matching the product's live-check identity.
- **License Expiry card:** a countdown ring (days until license renewal needed), same 
  sweep-fill animated gauge treatment used elsewhere in Trustline for visual consistency.
- **Check History card:** a small vertical timeline of the last few automated checks run 
  (date + result), showing this is continuous monitoring, not a one-time check.

Include a "Re-check now" button that re-triggers the status check with a brief loading 
state on the hero card specifically (not a full-page reload).
...
`GET /trustline/vendor/{vendor_id}/sam-licensing-status` and 
`POST /trustline/vendor/{vendor_id}/sam-licensing-recheck`. Deterministic — no LLM. In a 
real system this would call the actual SAM.gov API; for this file, implement a clearly-
commented mock function `check_sam_gov_status(vendor_id)` returning a plausible status, 
and note in a comment exactly where the real SAM.gov API call would go (SAM.gov does have a 
public entity-search API — mention its existence in the comment even if you mock the call 
for the hackathon). Seed Meridian Steel as active/eligible with license expiring in 45 
days, and seed a check-history list of 3-4 past automated checks.
