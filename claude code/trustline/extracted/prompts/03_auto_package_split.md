...
This one is event-driven, not a static dashboard — visualize an order being redistributed.

**Grid layout (3 cards, sequential story left to right):**
- **Original Order card:** shows the failed order (e.g. "Meridian Steel — 200 units, 
  Failed: can only deliver 60%") as one solid bar.
- **Split Animation (col-span-2, the hero):** an animated stacked-bar visualization — the 
  original bar visibly splits into segments (e.g. 60% stays with Meridian, 25% redirects to 
  Titan Fabricators, 15% to a third backup), each segment animating outward and re-coloring 
  to its destination vendor, with a connecting line from each segment down to a small 
  vendor-name label. This split animation is the signature moment for this feature — build 
  it with real motion (segments literally separate and travel), not a static post-split 
  chart.
- **Schedule Impact card:** a small before/after comparison — "Original risk: 12 days late" 
  vs "After split: 2 days late" — with a --success color on the improved number.
...
`POST /trustline/auto-package-split`. Input: `failed_order: {vendor_id, item, quantity}`. 
Deterministic allocation logic: query backup candidates (use Titan Fabricators and one more 
seeded vendor) ranked by trust score and relevant capability, greedily allocate the 
shortfall quantity across them up to each one's plausible capacity (seed a capacity field), 
return the split plan plus a recomputed schedule-impact estimate (simple heuristic: 
shortfall_quantity × some per-unit delay factor, comment that this is a simplified model). 
No LLM needed — this is allocation logic, not reasoning about text.
