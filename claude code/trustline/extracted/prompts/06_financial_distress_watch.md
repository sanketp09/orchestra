# 6. Financial Distress Watch — 2 files
...
Use Coastal Bolt & Fastener (the seeded distressed vendor) as the primary demo subject here 
— this screen needs a vendor with real flags to be worth showing.

**Grid layout (4 cards):**
- **Hero card (col-span-2, the glass/featured card):** a horizontal filing timeline — a 
  timeline strip showing UCC filings, liens, and judgments as dots/icons plotted along a 
  date axis, each colored by type, animating in left-to-right in sequence (not all at once) 
  to feel like a live record being assembled.
- **Risk Level card:** a large gauge/dial (not a plain badge) sweeping to a risk position 
  (Low/Medium/High/Critical zones colored along the arc), animated sweep.
- **Active Filings card:** an animated counter of currently active filings.
- **Total Exposure card:** an animated dollar-figure counter representing estimated 
  financial exposure across the flagged filings.
...
`GET /trustline/vendor/{vendor_id}/financial-distress`. Pure deterministic — no LLM. Seed 
a small `FILINGS` list for `vendor_coastal_bolt` (2-3 filings: a UCC filing, a mechanic's 
lien, with dates and dollar amounts) and compute: `risk_level` (derived from filing count + 
recency — e.g. 2+ filings in the last 6 months = High), `active_filings_count`, 
`total_exposure` (sum of filing amounts). `needs_human=True` whenever risk_level is High or 
Critical. Also seed Meridian Steel with exactly one UCC filing (matching the earlier 
central dataset) so this endpoint returns something for both demo vendors, at different 
severities.
