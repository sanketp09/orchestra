# 9. Hidden Ownership Detector — 2 files
...
**Grid layout (2 cards, deliberately asymmetric):**
- **Hero card (col-span-3, the glass card, full width):** a small node-link network 
  diagram (hand-built SVG, not a heavy graph library) — two or three vendor-name nodes, 
  connected by labeled edges when they share a registered officer or address (e.g. 
  "Meridian Steel" and a fictional "MSF Holdings LLC" connected by an edge labeled "Same 
  registered agent"). Nodes and edges animate in — edges draw via stroke-dashoffset, nodes 
  fade+scale — this diagram IS the finding, make it the clear visual focus of the page.
- **Supporting Evidence card (below, full width):** a simple table/list of the exact 
  matched fields (address, officer name) side by side for the connected entities, so the 
  network diagram's claim is immediately backed by literal evidence underneath it.
...
`GET /trustline/vendor/{vendor_id}/ownership-check`. Deterministic string matching (exact 
and simple fuzzy match via difflib) across a seeded `VENDOR_REGISTRATIONS` table (registered 
address, registered officer per vendor) — no LLM needed. Seed at least one genuine match 
(e.g. two differently-named entities sharing a registered agent) so the network diagram has 
something real to render. Extend the base model with `connected_entities: list[{entity_
name, shared_field, shared_value}]`. `needs_human=True` whenever any match is found.
