# 1. Supplier Digital Twin — 2 files
...
This is the master vendor profile — the bento grid that ties all other Trustline features 
together into one page for Meridian Steel.

**Grid layout (7 cards):**
- **Hero card (col-span-2, row-span-2):** the trust trajectory area/line chart (2023→Today), 
  the glass-treatment card for this screen, with the current score (76) large and animated 
  at top, a trend arrow, and the "Updated Xs ago — [reason]" tooltip line.
- **Response Time card:** a small sparkline of average response time trending upward (bad), 
  with the current value ("4.2 days avg, up from 1 day").
- **Change-Order Pattern card:** a small horizontal bar comparing this vendor's typical 
  change-order markup % against a portfolio-average bar beside it.
- **Bonding Capacity card:** a radial gauge showing $2.4M / $4M utilized, animated sweep-
  fill, percentage in the center.
- **Safety Trend card:** a mini line chart of EMR over the last 3 periods, with a small 
  "1.08 vs 1.00 industry avg" label.
- **Evidence Count card:** a simple large animated counter of total linked evidence items, 
  with a "View all →" link to the Receipt Center.
- **Recent Changes card (col-span-2):** a horizontal timeline strip (not a vertical list 
  here — this is the bento/infographic version) of the 3 most recent events, each a small 
  dot+icon+date, clickable to expand inline.

All cards animate in with a staggered bento-reveal (cards fade+scale from 0.96 to 1, 
staggered by grid position, ~60ms apart). Include a "Simulate new evidence" demo trigger 
button that fires the trust-score count-down animation live (82→76 equivalent from current 
state) — this is your rehearsed live-demo moment, make it reliable.
...
`GET /trustline/vendor/{vendor_id}/twin` — aggregates all dimensions of TrustBelief for one 
vendor into one response (schedule_reliability, financial_stability, safety, 
responsiveness), plus the trust trajectory history and recent evidence events. This is 
purely an aggregation/read endpoint — it queries results that the other 9 Trustline 
features' own endpoints would have written, it does not recompute anything itself. Seed 
enough mock underlying data for `vendor_meridian_steel` that this aggregation has 
something real to assemble from all 7 card slots described above.
