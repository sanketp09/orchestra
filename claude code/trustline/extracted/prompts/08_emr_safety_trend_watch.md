# 8. EMR & Safety Trend Watch — 2 files
...
**Grid layout (4 cards):**
- **Hero card (col-span-2, row-span-2, glass treatment):** a line chart of EMR over the 
  last 3-4 periods, with a second, distinctly-styled reference line for the industry 
  average (1.00) overlaid so the comparison is immediately visible — use a dashed line 
  style and a muted color for the benchmark line versus a solid --accent line for this 
  vendor's actual EMR.
- **Current EMR card:** a large number (1.08) with a small "vs. 1.00 industry avg" 
  subtitle and a --warning tint since it's above benchmark.
- **OSHA Incidents card:** a small bar chart of recordable incidents per period (mostly 
  zero bars, one period with a bar) rather than just a number — this makes "one incident 8 
  months ago" visually contextualized against a mostly-clean record.
- **Safety Score card:** a composite radial gauge (0-100) combining EMR trend + incident 
  recency into one score, animated sweep-fill.
...
`GET /trustline/vendor/{vendor_id}/safety-trend`. Pure deterministic — no LLM. Seed a small 
EMR history list for Meridian Steel and one recorded OSHA incident 8 months ago (matching 
the central dataset). Compute a composite `safety_score` via a simple weighted formula 
(e.g. penalize EMR above 1.0 proportionally, penalize incident recency inversely with time 
elapsed) — comment the formula clearly since it's a simplified model, not a real actuarial 
calculation. `needs_human=True` only if EMR trend is worsening AND a recordable incident 
occurred within the last 12 months (both conditions, not just one).
