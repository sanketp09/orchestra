...
export default function DecisionDebtLedger() {
  const [selectedId, setSelectedId] = useState<string | null>(RAW_DEBTS[0].id);
  ...
  const stack = useMemo(() => {
    const open = hydrated.filter((h) => h.status !== "resolved");
    return {
      openDecisions: open.length,
      totalExposure: open.reduce((s, h) => s + h.debt.exposure, 0),
      overdueCount: open.filter((h) => h.status === "overdue").length,
      criticalCount: open.filter((h) => h.critical).length,
    };
  }, [hydrated]);

  const timelineRange = useMemo(() => {
    const times = RAW_DEBTS.map((d) => d.deadline.getTime());
  ...
  function handleSimulate(debtId: string) {
    const debt = RAW_DEBTS.find((d) => d.id === debtId)!;
  ...
  const selected = hydrated.find((h) => h.debt.id === selectedId) ?? hydrated[0];
    ...
    >
      ...
      <div className="mb-8">
        ...
        <div
          ...
          style={{ boxShadow: "-3px -3px 8px rgba(255,255,255,0.55), 4px 4px 10px rgba(12,9,4,0.08)", minHeight: 160 }}
        >
          ...
          {loaded &&
            hydrated.map(({ debt, status, critical }) => (
              <DebtNode
                key={debt.id}
                debt={debt}
                status={status}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.1fr] gap-6">
        <div>
          ...
          <div className="flex flex-col gap-2">
            {hydrated
              .slice()
              .sort((a, b) => (b.critical ? 1 : 0) - (a.critical ? 1 : 0) || b.debt.exposure - a.debt.exposure)
              .map(({ debt, status, critical, daysRemaining }, i) => {
                const meta = STATUS_META[status];
                return (
                  <motion.button
                    key={debt.id}
                    ...
                    style={{
                      boxShadow: "-2px -2px 6px rgba(255,255,255,0.5), 3px 3px 8px rgba(12,9,4,0.07)",
