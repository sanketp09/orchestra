...
// Root screen
// ---------------------------------------------------------------------------
export default function ProcurementXRay() {
  ...
  useEffect(() => {
    ...
    const findingTimers = MOCK_FINDINGS.map((finding, idx) =>
      window.setTimeout(() => {
    ...
    const finishTimer = window.setTimeout(() => {
      setScreen("results");
    }, 700 + MOCK_FINDINGS.length * 480 + 500);
  ...
  const futureDisputes = MOCK_FINDINGS.filter(
    (f) => f.impact === "high" || (f.type === "clause_risk" && f.needsHuman)
  ).length;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {(screen === "scanning" || screen === "results") && (
          <div className="grid gap-4" style={{ gridTemplateColumns: "280px 1fr 340px" }}>
            ...
            />
            <XRayScanPanel screen={screen} activeLayerIndex={activeLayerIndex} findings={MOCK_FINDINGS} />
            <FindingsPanel
              screen={screen}
              revealedFindings={screen === "scanning" ? revealedFindings : MOCK_FINDINGS}
        ...
        <AnimatePresence>
          {screen === "results" && (
            <motion.div
              ...
              className="mt-4"
            >
              <DisputeBanner count={futureDisputes} onSelectFirst={() => setSelectedFinding(MOCK_FINDINGS.find((f) => f.impact === "high") ?? null)} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: tokens.mutedForeground }}>
            Compass will x-ray it against drawings, BOQ, vendor history, and pricing evidence.
          </p>
        </div>
        <Button
          onClick={onStart}
        ...
        {MOCK_DOCUMENT_LINES.map((line) => {
          ...
          return (
            <motion.div
              key={line.id}
              animate={{
                backgroundColor: isHighlighted ? "rgba(172,114,62,0.16)" : "rgba(172,114,62,0)",
            ...
            return (
              <motion.div
                ...
                style={{
                  ...
                  background: passed || active ? "rgba(172,114,62,0.06)" : "transparent",
      ...
      {finding && (
        <>
          ...
          <motion.div
            ...
            style={{ width: 420, background: tokens.surfaceCard, boxShadow: "-8px 0 24px rgba(12,9,4,0.15)" }}
            initial={{ x: 420 }}
            animate={{ x: 0 }}
            exit={{ x: 420 }}
